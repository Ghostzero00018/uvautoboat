...
| Verify Pi 5 MAVProxy heartbeat endpoint | 🟡 02/06/2026 (shown 03/06) — professor photo sent on 03/06 morning, with the Pi desktop clock showing Tue 02/06/2026 22:09 capture time, shows MAVProxy on `/dev/ttyAMA0` at `57600` detecting vehicle `1:1` on link 0, `online system 1`, mode `HOLD`, `fence present`, and ArduPilot `EKF3 waiting for GPS config data` status text. This upgrades the endpoint from missing to externally evidenced for MAVProxy, but MAVROS `/mavros/state connected: true` and first ROS telemetry still need a pass-condition check. |
...
**Document Version**: 9.19 | **Last Updated**: 03/06/2026
