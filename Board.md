...
| Run isolated Pi 5 YOLO CPU feasibility spike (no CCU required) | ✅ 09/06/2026 + 10/06/2026 — `yolo26n.pt` loaded in `~/venvs/yolo-pi5`, exported to NCNN as `yolo26n_ncnn_model`, and ran static-image CPU inference at `imgsz=320` on the `bus.jpg` fallback with `detections: 5` and speed timings preprocess 17.24 ms / inference 244.42 ms / postprocess 15.05 ms. The 10/06 demo repeated static-image NCNN inference for 5 runs on local `bus.jpg`, with 5 detections each run, mean inference `84.09` ms, and temp 68.8 C -> 72.2 C. No RealSense capture, ROS integration, dashboard integration, camera-stream pipeline, or command-path work was run; this remains Pi-local static-image feasibility only. |
...
**Document Version**: 9.25 | **Last Updated**: 18/06/2026
