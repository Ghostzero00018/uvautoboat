...
| 02/09/2026 | **Pi bundle deployed and certified at `778e069`:** published revision transferred to `/home/imt-aqua-drone/uvautoboat_real_fcu_bundle_20260902_778e069` and certified non-actuating. Pi powered; no flight controller, propulsion or other hardware powered, nothing actuated, no runtime started. Source parity held first - clean worktree, `HEAD` and `origin/main` both `778e069263044de67f625bdc8de73ac54271d3a0` - and the four governed members are byte-identical to `0ed5525`, the intervening commit being documentation only. Pi preconditions passed with no `MISSING` line and no `SERIAL BUSY`. The unpowered flight controller does not block certification: `check` requires `/dev/ttyAMA0` to exist, be read-write and be free but never opens a link. The manifest digest read `fedda913bb31698b150f29a96fd82735637e04d5a6ea2b8a573e39724310db58`, matching the workstation, and the four governed hashes verified twice - once explicitly, once by `rfcu_pi_verify_bundle` inside `static_preflight` - before `REAL_FCU_PI_CHECK=PASS serial=/dev/ttyAMA0 runtime=not-started`. A second pass retained `/home/ghostzero/Desktop/pi_bundle_certification_20260902_778e069.log` and reproduced the marker, and every digest the Pi reported was compared on the workstation against both the worktree and `HEAD` rather than read by eye; all five matched and the governed lines are identical to the manifest. The Pi entry reads `43a4775f`, the value the manifest moved to after the guard change, so the fail-closed runtime is what is deployed. This was also the first hardware execution of that change: the twelve guards in `static_preflight` ran twice with no spurious abort, while the remaining `111`, including the thirteen in `rfcu_pi_run`, still require a run mode. Transfer was direct `ssh`/`scp` from the workstation, operator-run under password authentication; the 01/09/2026 one-shot helpers no longer exist in the tree. Classification: **DEPLOYED / CERTIFIED / NOT RUN**; this grants no run authority. | 🟡 |
| 02/09/2026 | **Hardware-safety reading documented for operators, and a fail-open finding in the view-only Pi monitor:** the runbook gained "Hardware-safety reading on the dashboard" - the three rendered strings, the read-only contract, and the properties that matter during a run: `Unknown (stale)` is not `ENGAGED` and proves nothing either way, `RELEASED (suppression off)` never means propulsion is powered or the vehicle armed, and the reading needs no `enable_fcu_bench_control=1`. Two cross-references were added where the document already points the operator at the switch, both framing the reading as a cross-check and never authority, with a contradiction or a stuck `Unknown (stale)` treated as no observation. A first draft claimed no gate in any helper consults the safety state; that was false and was narrowed before shipping to what a whitelist test actually proves. Narrowing it surfaced a finding, recorded and **not repaired**: `tools/pi_live_hailo_mavlink_dashboard.sh:2274` derives its own `hardware_safety_is_on()` with neither the staleness nor the boolean guard the bridge applies, and `latest_sys_status` carries no timestamp, so a stopped `/mavros/sys_status` stream lets the last sample answer indefinitely. Both failure modes push toward "safety is on", which `restored_safe_state()` requires, and that drives the `COMPLETE` transition at line `2313` and the `FINAL_STATE_LOST` abort at `2317` - so a stale sample can call an armed observation complete and suppress an abort that should have fired. Same class as the badge staleness fixed on 01/09/2026, in a helper not swept then. The file is checksum-pinned at `0d3f6d1b` and `95,720` bytes and is a Pi-side safety monitor, so a repair needs an explicit decision and a re-pin. Recorded before any armed run. | 🟡 |
| 02/09/2026 | **Correction - the Pi monitor staleness finding is withdrawn:** the fail-open finding recorded against `tools/pi_live_hailo_mavlink_dashboard.sh` in the entry above does not hold, and no change was made to that file. `hardware_safety_is_on()` does apply no staleness bound of its own, but that does not reach a fail-open: `/mavros/sys_status` is a required subscription at line `2216`, `record_required` timestamps it in `last_seen`, and all three `restored_safe_state()` call sites sit behind `stale_topic` - the `WAIT_BASELINE` transition at `2297` is a short-circuit `and` whose second term is `self.stale_topic(now) is None`, and the `FINALIZING` and `COMPLETE` branches at `2313` and `2317` are reachable only past the `abort_seen("REQUIRED_TOPIC_STALE", stale)` return at `2302`. A stalled stream aborts the observation before the safety reading is consulted, so the monitor is fail-closed on this path with the guard in the caller rather than the reader. The boolean case is unreachable: `sensors_enabled` is a `uint32` field and an `int` in `rclpy`. The review error was generalising a missing guard in one method into a system property without tracing its callers, and assuming rather than checking the comparison with the bridge, which needs its own rejection because it publishes a display value with no caller-side abort. The file is unchanged and still matches its runbook pin at `0d3f6d1b` and `95,720` bytes; no re-pin was required. | 🟡 |
| 02/09/2026 | **Run-path guard map, corrected expectation and a pre-run parameter contract:** prepared for a single full-stack run. Correction first - the claim that the new bundle is stricter and may stop a run that previously proceeded is withdrawn for the normal path. `rfcu_pi_main` dispatches `rfcu_pi_run` as a plain statement in a `case` branch under `set -euo pipefail`, and each `rfcu_pi_fail` is the last command of its or-list, so `set -e` already aborted on all thirteen guards before the change; verified against the same shape, which exits `1` and prints no continuation. The fail-closed change is robustness, not new strictness. The thirteen stop points in `rfcu_pi_run` lines `1635-1759` are mapped with message and trigger: mode dispatch `1655`, Hailo readiness `1680`, T0b probe `1688`/`1691`/`1692`, full MAVROS `1698`, parameter guard `1704`, T3a propulsion `1711`, manual safety release `1723`, post-release readiness `1731`, and workstation discovery `1734`/`1738`/`1740` as three messages for one failure. The parameter guard at `1704` is checkable in advance and its nine bounds are tabulated from the bridge source. Largest single risk to a one-shot run: `RC_OVERRIDE_TIME` must be in `(0, 0.5]`; it was `3.0` on 31/08/2026 and set to `0.5` on 01/09/2026 and left temporary with rollback explicitly open and no rollback artifact, so a current value of `3.0` ends the run at `1704`. The value has not been read since 01/09/2026 and can only be confirmed from the flight controller. Second historical run-burner recorded: the 21/08/2026 run ended at the `1688` equivalent with `47` of `50` samples connected and **armed**, so an already-armed FCU ends the run before the bridge exists. | 🟡 |
| 02/09/2026 | **Full-stack run-sheet recorded:** five workstation terminals plus the Pi, with domains, waiting markers and the stop order in one place, derived from what each supervisor waits on rather than from a previous run. Binding constraint: both workstation supervisors must be waiting before the Pi starts, because the Pi's discovery guards look for nodes the workstation side owns; W2's `start the approved real-FCU Pi helper now` message is therefore not the trigger, since W1 need not be up when it prints. Nothing forces W1 before W2 or the reverse, only both before the Pi. W1 and the Pi run domain `43` SUBNET, W2 domain `77` LOCALHOST with a relay onto `43`, and each helper exports its own domain. A gap is recorded explicitly: no supervisor starts `real_fcu_command_feedback_capture`, yet the `run-t3a` discovery guard requires it, so omitting that terminal stops the Pi at line `1738`; the node validates `ROS_DOMAIN_ID=43`, `ROS_AUTOMATIC_DISCOVERY_RANGE=SUBNET` and `ROS_LOCALHOST_ONLY=0` exactly and its tier positional must match the Pi's mode. Pre-run checks carried into the sheet: ports free, flight controller disarmed, and `RC_OVERRIDE_TIME` inside `(0, 0.5]`. Stop order is the reverse - externally disarm, stop W2, run the Pi closeout, stop W1 last, because W1's stop marker is what lets the Pi finish. The sheet carries no physical declaration values, and the order has never been executed end to end in this configuration, so a first attempt also tests the order. | 🟡 |
| 02/09/2026 | **T3a armed-run sheet recorded:** traced from `rfcu_pi_run` and the confirmation functions, extending the start-order sheet with where arming sits. The helper cannot arm - the MAVROS allowlist is `sys_status`, `param`, `global_position`, `imu`, `rc_io` with no arming plugin, and line `1753` states arming remains external. The arm point is after `REAL_FCU_T3A_READY=PASS`, never a starting condition: `rfcu_pi_wait_connected_disarmed` gates lines `1688` and `1698` on `connected` true and `armed` false, which is how the 21/08/2026 run ended with `47` of `50` samples connected and armed. Records a difference easy to be caught by: in `run-t3a` both `rfcu_pi_confirm_t3a_propulsion_enable` and `rfcu_pi_confirm_manual_safety_release` short-circuit and return success without prompting, because the ten approved flags are the declaration, so a flag set to `1` for a condition that is not true removes a gate that would otherwise have stopped the run; T2 modes prompt for exact tokens instead. The two external cues are the `operator_action=external` log lines for propulsion enable and safety release, both while disarmed. `REAL_FCU_PROPULSION_ISOLATED=1` describes the starting state; propulsion is enabled later during the run. Closeout does prompt, with a `300`-second default timeout and the exact token `NEUTRAL_ESTOP_FCU_DISARMED_SAFETY_ON_PROPULSION_ISOLATED`, in the stated order. No flag values, tier choice or go/no-go are recorded; those remain the operator's. | 🟡 |
| 02/09/2026 | **Pi terminal session-persistence note added to the T3a sheet:** an `ssh` session and a Remmina desktop terminal are not equivalent for a run ending in an interactive closeout. `rfcu_pi_start_child` launches children with `exec setsid` and stdin from `/dev/null`, so they survive the parent terminal, but the supervisor runs in that terminal; over `ssh` a dropped link `SIGHUP`s it, the closeout prompt at line `1433` is never answered and `rfcu_pi_cleanup` records `REAL_FCU_T3A_SAFE_CLOSEOUT=FAIL confirmation=missing-or-invalid` with `cleanup_rc=1`. An RDP desktop session persists across a client disconnect and the helper reads no `DISPLAY`, `WAYLAND_DISPLAY` or `xdg-user-dir`, so Remmina removes that failure mode at no cost. Three consequences recorded: paste the thirteen-flag line rather than retyping it, since a mistyped name leaves the variable unset and defaults to `0` so the gate stops the run - fail-safe but costly on a single attempt, with the Remmina `1.4.43` `deny_screenshot_clipboard=false` and Pi GNOME-RDP NLA plus `grdctl` credentials fix if the clipboard does not reach the Pi; do not close the Remmina window between arming and closeout, because the prompt lives on the supervisor's own terminal; and confirm `Ctrl+C` passes through the Remmina keyboard settings before the run, since the stop order depends on it. | 🟡 |
| 02/09/2026 | **Workstation dress rehearsal executed; five run-sheet defects found and corrected:** terminals 1 to 3 of the run-sheet run at `90a9cdd` with no Pi and no flight controller, to prove the workstation half and the start order before the single full-stack attempt. Five defects, all corrected in the sheet: W2 needs ten environment variables the sheet listed none of, surfaced one restart at a time as `FCU_VRX_LEFT_SERVO_CHANNEL is required` then `FCU_VRX_OBSERVER_STALE_SECONDS is required`; `run-real-fcu` additionally rejects the default `FCU_VRX_CORRELATED_OBSERVATION=0`; the capture command was invalid because `t3a` is rejected without `--esc-threshold-calibration`; that capture terminal is interactive rather than fire-and-forget, printing `REAL_FCU_ESC_THRESHOLD_INPUT=READY` and expecting typed observations; and W2 carries a `120`-second fuse after `PRESTART=PASS`, exiting `status=1` with `real-FCU RCOut relay did not become ready before the deadline` and tearing down VRX with it. What passed: W2 reached `PRESTART=PASS` with `mapping=3/1 rails=800/800/2200` matching a previous run's marker, VRX ready, observer fail-closed, pose baseline on `sydney_regatta`, bridge and relay started; W1 reached `REAL_FCU_WORKSTATION_SERVICES=PASS ports=8002,9090` and its waiting state with the domain `43` graph showing exactly `/rosapi` and `/rosbridge_websocket`; capture reached `REAL_FCU_CAPTURE_READY=PASS tier=T3A subscriptions=5`. Teardown clean both ways, `cleanup_rc=0`, no survivors, ports released, worktree unchanged. The `pgrep -af` conflict false positive was also reproduced: the preflight reported a conflicting process with none running, because the guard matched a sweep command's own argv, and passed on a clean command line. Not covered: the Hailo branch, which blocks on Pi-sourced `/perception/detections`, and everything Pi- or FCU-side. | 🟡 |
| 02/09/2026 | **Run-sheet rewritten for the bidirectional twin and the readiness timeout:** the sheet described only the outbound leg, which proves half a demo. Both legs are now documented with their exact gates. Outbound: dashboard demand through the Pi bridge to the flight controller, measured `/mavros/rc/out` on domain `43` read by the relay, forwarded over `127.0.0.1:14555` and converted PWM to thrust by the domain `77` bridge onto the two `/wamv/thrusters` thrust topics. Return: VRX pose and thrust emitted as twin telemetry over `127.0.0.1:14556` onto `/fcu_to_vrx/twin_telemetry` on domain `43`, schema `uvautoboat.fcu_to_vrx.twin_telemetry.v1`, rendered by the dashboard and expiring after `2000` ms. Three post-Pi markers are tabulated in order - `FCU_TO_VRX_RC_OUT_RELAY_READY=PASS`, `FCU_TO_VRX_TWIN_TELEMETRY_READY=PASS` and `FCU_TO_VRX_WORKSTATION_READY=PASS ... observer=ready streams=4` - with the note that the end-to-end proof is the dashboard twin-telemetry panel rather than any marker, and that `rails=800/800/2200` puts neutral at minimum so neutral demand reads zero VRX thrust. The W2 command now sets `FCU_TO_VRX_READY_TIMEOUT_SECONDS=600`: the `120` default bounds six waits and its window opens at `PRESTART=PASS`, before W1, the capture terminal, the browser and the Pi are started, and the Pi publishes RCOut only after its probe cycle and full MAVROS reach connected and disarmed, so `120` is not achievable by hand. The four-stream observer gate is bounded separately at `900`. `RC_OVERRIDE_TIME` was operator-confirmed at `0.5`, clearing the largest line-`1704` hazard. | 🟡 |
| 02/09/2026 | **First full-stack T3a attempt stopped at gate `1688`; link proven healthy, two record corrections:** the stack ran at `600303e` with bundle `778e069` and propellers fitted, reached the T0b probe and stopped. No arm, no propulsion command, no actuation. Large parts passed for the first time: the Hailo preflight executed on hardware, `17` guards taking hardware-verified coverage from `12` to about `29`; all thirteen T3a flag gates passed; and the entire unrehearsed Hailo handshake worked end to end, Pi detections through W1 starting `web-video-server` and `person-stop-monitor` to `SERVICES=PASS ports=8002,8080,9090`. The stop was `/mavros/state` reading `connected: false` with a stale stamp. Three diagnoses were proposed and all three were wrong: floating-line noise or baud mismatch, disproved by a hex dump showing clean MAVLink2 frames with constant `sysid=1 compid=1`; drifted FCU serial parameters, disproved by operator readback matching the 03/08/2026 baseline exactly; and a T0b allowlist unable to publish `/mavros/state`, disproved by `sys_status` being present. An isolated MAVROS run then connected in about `1.5` seconds and completed a `986`-parameter pull. Two corrections to the retained record follow: the Pi-to-FCU request/response defect carried open since 03/08/2026 does not reproduce, and implausible router addresses are not a fault signature since they appear on a healthy link - the failure signature is the absence of `1.1`. A step zero was added to the run-sheet: prove the link with that isolated MAVROS run, which answers in seconds what gate `1688` takes up to `600` to report. Gate `1688` behaved correctly throughout, refusing a props-fitted run whose telemetry link was not established; the cost was time, not safety. | 🟡 |
| 02/09/2026 | **Full-stack T3a digital-twin run completed, and the throttle ceiling raised to `0.20`:** the second attempt reached readiness on both sides, closed the bidirectional twin loop against the real flight controller and ended on a confirmed safe closeout - the first time the full path has run at any revision. Two diagnosed changes unblocked it. The Hailo child was removed: with it enabled the probe logged `78` implausible router addresses and never `1.1`; the Hailo bridge does not open the serial port, so contention is ruled out and the mechanism remains unknown. And guard-snapshot mode replaced the live parameter pull, whose `timeout 20` cannot fit this vehicle's `986` parameters at a measured `36` seconds - `rfcu_pi_capture_runtime_guard` routes to `rfcu_pi_capture_snapshot_guard` and skips the pull, which is how the 01/09/2026 run succeeded; its artifact was still valid and was checked against every bound the bridge enforces before use. The guard resolved `left=SERVO3 right=SERVO1` from the vehicle's own parameters, the reverse of the simulator's. Both legs closed: `RC_OUT_RELAY_READY`, `TWIN_TELEMETRY_READY`, then `WORKSTATION_READY ... observer=ready streams=4`, with capture recording all four streams received. Closeout gave `SAFE_CLOSEOUT=PASS` and `FINAL_STATE=PASS connected=true armed=false`, and W1 exited `status=0`. Separately the throttle ceiling moved from `0.12` to `0.20` across five enforcing surfaces, two of them bundle members; three independent guards caught the change including a deliberate helper tripwire, and five test assertions were updated to exercise the new endpoint rather than merely expect a different number. Steering and throttle now share `0.20`, a `67%` increase in maximum commanded throttle on a props-fitted hull; the number was operator-specified. Manifest regenerated to `0e7395f5` and `14944a50`, so the deployed bundle is stale and requires a fresh transfer and certification. | 🟡 |
| 02/09/2026 | **Person-alert advisory mode, an explicit opt-in that stops nothing:** `REAL_FCU_PERSON_ALERT_ADVISORY=1` on the Pi, defaulting off and refused without `REAL_FCU_HAILO_PERSON_STOP=1`, makes a detected person publish and warn without latching an emergency stop or withholding readiness. The freshness requirement is deliberately kept: a stale or dead detector feed still stops in either mode, so advisory means a person is present and we are continuing, not that the detector is optional. The operator specified this after the consequence was stated. The first implementation changed only the alert callback and was incomplete: the primary stop is in `_tick`, which latches every cycle while armed, and would have stopped the boat in the field while passing its own tests; freshness and the hold are now separate terms there with only the hold bypassed, covered by a test proved load-bearing against a reverted gate. The same pass removed a duplicate `person_hold_clear` payload key that Python was silently resolving to the last occurrence. The dashboard would otherwise have claimed `PERSON OBSTACLE - STOPPED` while the propellers turned; it now reads `ADVISORY, NOT STOPPED`, still critical, and only on an affirmative fresh report - no bridge, a stale status or a missing field all keep the stop wording, so simulation is untouched. A first attempt dropped the stop claim whenever bench status was absent, broke ten tests and was corrected. The T3a READY marker now ends `person_alert=advisory-no-stop` or `person_alert=stop-enabled`. Coverage: bridge `63` to `67`, helper `69` to `73`, dashboard `96` to `101`; a harness clock-mocking defect was found and fixed while writing them. Both bundle members changed again to `fd9d0250` and `50e2eac3`, manifest `f0a91bb4`, so the `65e1fb8` bundle certified this afternoon is stale and needs a fresh transfer. | 🟡 |
| 02/09/2026 | **ESC start threshold measured, and a calibration-interface finding:** during the armed T3a run on bundle `929831e`, with steering at `0`, both propellers begin turning from rest at demand `0.15`, measured `996 us`, `+14.0%` of the `800/800/2200` rail; the same run showed them turning at the `0.20` ceiling at `1066 us`, `+19.0%`. The former `0.12` ceiling capped output at `968 us`, which is `28 us` below the start threshold, so no throttle under the previous limit could ever have turned the motors - the request to raise it was correct and the margin was narrow. An earlier claim in this session that the threshold sat at the ceiling with no usable band is withdrawn: from-rest start at `0.15` against a `0.20` ceiling leaves about `70 us`, or `0.05` in demand units, of controllable range, which is narrow but real. Recorded as a finding and not repaired: the calibration interface correlates an operator observation to the last ACTIVE plateau within a `10`-second grace, which works, but records the plateau held at release rather than the transition, so locating a threshold becomes a bisection costing a hold, release and type cycle per candidate; and the terminal observation is one-shot per side, so an entry made while holding `0.20` pins the artifact at `1066 us` and only a fresh capture session can record `996 us`. The mechanism is sound for confirming a known value and poor for discovering one. | 🟡 |
| 02/09/2026 | **T3a run closeout and capture verdict:** both supervisors were stopped in the documented order and both exited `status=0 cleanup_rc=0`, the first fully clean teardown of the day; a survivor sweep found nothing and ports `8002`, `8080` and `9090` were free afterwards. The Pi wrote `NEUTRAL_ESTOP_FCU_DISARMED_SAFETY_ON_PROPULSION_ISOLATED` and its run directory `real_fcu_t3a_pi_20260902_180626` was copied to `/home/ghostzero/Desktop/pi_run_evidence/`. The capture artifact records `59m18s` armed, `106,339` events, `invalid_status_count 0`, publisher binding `pass=true` observed as `/real_fcu_rc_command_bridge_t3a`, and a final latched `EMERGENCY_STOP` with hardware safety engaged, disarmed and both servos at `800`. `236` state entries alternate `115` `ACTIVE` with `116` `ARMED_NEUTRAL`. The verdict is nevertheless `pass:false`: `calibration` is `null` on both sides and `operator/esc_threshold` counted `0` events, so the `996 us` measurement exists only as prose and not in the artifact. That is the calibration-interface cost made concrete, not a failed measurement. | 🟡 |
| 02/09/2026 | **Single workstation entry point, and two findings:** `tools/real_fcu_full_stack_workstation.sh` with suite `tools/test_real_fcu_full_stack_workstation.sh` replaces the three workstation terminals with one command, sequencing the VRX supervisor, the real-FCU supervisor and the capture node, the last in the foreground because it is interactive and the run above failed its verdict only for want of typed observations. It re-implements no guard; a stop still prints the owning helper's reason. Start order was verified from source rather than from the run-sheet: the real-FCU supervisor has no domain-emptiness check and none of the VRX supervisor's children matches its ten conflict patterns. Finding one: a supervisor started as a plain background job inherits `SIGINT` ignored and cannot trap it, measured as `SigIgn=...0006` with `SIGINT` uncaught, so its stop handler never runs and the stop falls through to a `TERM` escalation that both supervisors record as failed. Job control gives `SigIgn=...0004` with `SIGINT` caught; `setsid` is no longer used because under job control it forks and the recorded pid stops referring to the supervisor. Covered as a regression, proven by reintroducing the defect. Finding two: `tools/test_fcu_to_vrx_workstation.sh` is non-hermetic, giving `PASS cases=33` scrubbed and `missing live-read configuration was accepted` with the eleven `FCU_VRX_*` values exported. It is recorded, not repaired, and the entry point scrubs before delegating. Workstation-only: no Pi, flight controller, simulator or hardware runtime was started, and the entry point has not met either. | 🟡 |
| 02/09/2026 | **FCU-to-VRX suite made hermetic:** `bash tools/fcu_to_vrx_workstation.sh check` now passes from a shell carrying the eleven `FCU_VRX_*` values, where it previously stopped at `missing live-read configuration was accepted`. Same mechanism as the Pi helper suite this morning: cases asserting that a **missing** live-read value is rejected run the check in a subshell that inherits the operator's environment, so an exported value reaches the assertion and validation succeeds where the case requires failure. The supervisor was never wrong; the suite was reading the shell it was started from. `fcuvrx_test_scrub_ambient_env` now unsets the whole `FCU_VRX_*` and `FCU_TO_VRX_*` families once before any case. Three cases added, `33` to `36`: a probe re-executing this file with a reserved argument so the assertion binds to the real entry call rather than a separate invocation, a control asserting the polluted fixture still passes validation so the probe cannot rot into a vacuous pass, and a polluted nested run that must still reach the final `PASS cases=` marker. Reserved tokens are matched before the first positional is read as the supervisor path, and are arguments rather than environment variables because an environment trigger would itself be an ambient bypass. Two mutants were run: deleting the entry call is caught by the pre-existing case, and narrowing the scrub to `FCU_VRX_*` alone is caught only by the new probe, which names `FCU_TO_VRX_READY_TIMEOUT_SECONDS`; no pre-existing case exercises that family, so the probe is load-bearing. Bumping the check marker edited the supervisor and made its pins stale, which the suite caught: the runbook SHA-256 is now `94d1825` and the size is unchanged at `35,799` bytes. Measured both ways after the change: `PASS cases=36` and `CHECK=PASS shell_cases=36 python_tests=48` from clean and polluted shells alike. A stale comment in the single workstation entry point, asserting the old `cases=33` and the old failure, was corrected. Workstation-only: nothing started. | 🟡 |
| 03/09/2026 | **First Hailo-enabled READY on hardware, three defects found live and fixed, Pi window added:** from one command per machine on bundle `929831e`, the Pi reached `REAL_FCU_T3A_READY=PASS ... hailo=ready person_alert=advisory-no-stop`, W2 reported relay, twin telemetry and the four-stream observer `READY`, and W1 reported all six telemetry topics plus `hailo=image,person-detections`. Before that, a wrong UART call: `/proc/tty/driver/ttyAMA` counts nothing while no process holds the port because the PL011 receiver is disabled, and `cat` opens at the boot-default rate, so a static `rx` and an empty `xxd` on a fresh boot were readings of a disabled receiver and a wrong-rate reader; the link was fine and MAVROS connected at once. Defect one: with advisory on, a person latched `EMERGENCY_STOP` anyway because W1 launched `person_stop_monitor.py` with `latch_emergency_stop:=true` and that node publishes on `/planning/emergency_stop`, the topic the dashboard E-stop also uses, so the bridge must keep honouring it; W1 now reads `REAL_FCU_PERSON_ALERT_ADVISORY` and turns the monitor latch off, four cases added and proved against a mutant. Defect two: on the first stop the Pi closed cleanly (`SAFE_CLOSEOUT=PASS`, `FINAL_STATE=PASS`, marker received, `status=0 cleanup_rc=0`) and W1 exited cleanly in three seconds, yet the entry point waited its whole 180 s grace, TERMed a bystander and reported `status=0`: the ROS 2 CLI daemon spawned by `ros2 topic echo` inherits the spawning process group on Linux, proven with a throwaway domain, so group liveness lied. Liveness is now the supervisor pid, stragglers are swept by pgid, W1's grace covers its marker wait, a forced stop exits non-zero with `stop=escalated`; two cases added and proved against a mutant. Feature: `REAL_FCU_HAILO_LOCAL_DISPLAY=1` drops `--no-display` so hailo-apps draws its native resizable window on the Pi desktop, with `DISPLAY` required at preflight; three cases added. GPS read `status -1` with zero satellites indoors, not a software fault. Helper suite `73` to `79`, wrapper suite `12` to `14`, manifest regenerated, committed `7cf684b`, transferred to `/home/imt-aqua-drone/uvautoboat_real_fcu_bundle_20260903_7cf684b`; certification and restart pending at time of writing. | 🟡 |
| 03/09/2026 | **Advisory mode proven on hardware; stale latched state read fixed; Pi window resizable:** the restart on `7cf684b` stopped at the snapshot guard with `connected:false` although MAVROS logged no lost connection; the guard's sample was stamped three seconds **older** than the probe's earlier `true`, the signature of a latched `/mavros/state` backlog read `--once`. The probe gate already retried past this, and yesterday's attempt 001 shows it doing so; the snapshot guard was single-shot. It now reuses the probe's retrying wait; two cases added and proved against a mutant. UART counters across the session, window open throughout: `rx` to `6,941,905`, `fe +36`, `brk +0`, `oe +1`, so the display does not touch the link and the overrun theory is closed. The wrapper pre-creates hailo-apps' `Output` window as `WINDOW_NORMAL` so it resizes, with a headless fallback; the entry point now tells the operator to start the Pi as soon as W1 starts in Hailo mode, because W1 waits for the Pi's detection feed. Suites `82` and `15`; committed `a8bed50`, transferred to `/home/imt-aqua-drone/uvautoboat_real_fcu_bundle_20260903_a8bed50` and certified. On it the gate passed first time, W1 was ready `34` s after the Pi's Hailo child, the Pi reached `T3A_READY ... person_alert=advisory-no-stop display=local-window`, and the window resizes. Armed with a person in frame, a `30` s joint capture at `/home/ghostzero/Desktop/advisory_proof_20260903_183827` holds `96` detections against `0` latches and `0` E-stop publications over `574` status samples: `ADVISORY_PROOF=PASS`. | 🟢 |
| 03/09/2026 | **ESC start threshold, operator-recorded reference:** on the armed `a8bed50` run with propellers fitted, from rest, throttle `0` with steering `-0.14` or `+0.14` starts the corresponding side at `994` us (`13.9 %` of the `800/800/2200` rail), and steering `0` with throttle `0.15` starts both at `996` us (`14.0 %`). Consistent with yesterday's `0.15 / 996` us. These operator observations are the reference; the capture node's typed calibration is not, and its `calibration` field is expected to stay `null`. Open: `t3a` capture still requires the calibration flag and fails its verdict on it, which should be decoupled. | 🟢 |
| 03/09/2026 | **Day close - the full showcase held for a 42-minute armed run:** Hailo live detection images on the workstation dashboard and, for the first time, in a resizable window on the Pi desktop; dashboard demand reached the fitted propellers through the Pi bridge (`2,214` demand frames, `9,841` measured RCOut samples, `65` `ACTIVE` plateaus) while the simulator followed and reported back (`streams=4`); person detection as an advisory, `96` detections against `0` latches and `0` `EMERGENCY_STOP` states in `73,808` events; Herelink arm and disarm reflected live on the dashboard (operator observed). Stop from one command: both supervisors `stopped cleanly after 5s`, `FULL_STACK_EXIT status=0 stop=clean`, Pi closeout `PASS`. Capture verdict `pass:false` on the two expected calibration reasons plus `final_status_not_disarmed` and `tier_status_sequence_incomplete`, both stop-order artefacts: the capture is stopped before the E-stop press in the Pi closeout, so it never records the terminal `EMERGENCY_STOP` the `t3a` check expects. Open: reword the stop order (E-stop first), decouple the calibration requirement from `pass`, fix the fresh-boot UART probe. | 🟢 |
| 03/09/2026 | **Correction - the Pi did not finish the stop unaided:** it stayed at its marker wait after W1 had exited with `STOP_MARKER=PASS`. W1's `ros2 topic pub --once --wait-matching-subscriptions 1` fires when the publisher counts a match, before the `VOLATILE` subscriber's side of the reliable match has necessarily completed, and W1 exits at once, so the marker can be lost with `PASS` recorded. The Pi's reader was found alive at `19:04:47` as the topic's only subscriber, the exact marker was republished by hand with matching QoS, and the reader exited on receipt. Proposed fix in W1: publish the marker several times over a short interval or as `TRANSIENT_LOCAL`; and the entry point's prompt should name the Pi line to wait for. Open. | 🟡 |
| 04/09/2026 | **Stop-path fixes proven on hardware, auto-move's first run, Hailo driver rebuilt after a Pi kernel update:** the Pi had booted `6.8.0-1064-raspi` with `hailo_pci` built for `1063` only, so the helper stopped at `/dev/hailo0 missing` while `lspci` still listed the Hailo-8; installing `linux-headers-6.8.0-1064-raspi` triggered the DKMS build and `modprobe` restored the node in six minutes, inside the stack's wait. On the second start both machines reached READY (`real_fcu_t3a_pi_20260904_163724`, `real_fcu_full_stack_20260904_163100`). The stop ended with the Pi receiving the marker burst unaided (`REAL_FCU_WORKSTATION_STOP=PASS marker=received`, `REAL_FCU_PI_EXIT status=0 cleanup_rc=0`), the workstation at `FULL_STACK_EXIT status=0 stop=clean`, and a verdict carrying only the two calibration reasons: both 03/09 stop defects closed. Auto-move ran by keyboard: straight then right at throttle `0.20`, steering `0.18` (left `1315` us, right `1066` us), the hull turned right, so the side label matches the mixer; `8` runs of `10` s or more. The operator recorded that on 03/09 and 04/09 the real propellers and the VRX twin moved together on the same command. | 🟢 |
...
| 04/09/2026 | **Forward correction - Hailo kernel-update wording:** the earlier same-day entry says that every kernel update removes the Hailo driver. The `4.24.0` package and source remained installed; the newly running `6.8.0-1064-raspi` kernel lacked matching headers and a built `hailo_pci` module. Installing those headers allowed DKMS to build the matching module. This wording correction does not change the recorded six-minute recovery or any hardware-run evidence. | 🟢 |
