...
Runs system checks across nodes, topics, parameters, and connectivity. Parameter values are reported as `PASS` (matches YAML baseline) or `TUNED` (user-applied via preset/dashboard — counted as healthy); `WARN` means a non-fatal issue such as unexpected drift, a slow probe, or skipped parameter checks; `FAIL` means a required check failed. Auto-detects IDLE/ACTIVE state. Also available from the dashboard Health Check panel with live-streaming output.
