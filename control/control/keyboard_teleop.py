...
class KeyboardTeleop(Node):
    ...
    def update_thrust(self):
        """
        Update thrust at 30 Hz based on throttle and rudder.
        ...
        print(
            f"\r{direction:10} | Throttle [{throttle_bar}] "
            f"{self.throttle*100:+4.0f}%{target_indicator:>6} | "
            f"Rudder [{rudder_bar}] {self.rudder*100:+4.0f}% | "
            f"L:{left_thrust:+5.0f} R:{right_thrust:+5.0f}   ",
            end='',
            flush=True,
        )
