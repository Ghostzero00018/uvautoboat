...
EXPECTED_HELPER_SHA256='0d3f6d1b72c473eeac8a169eaa045f24930ba7ed31fe4f7485d47616704f6ad9'
...
MAVROS_SOURCE_BATCH="${LIVE_MAVROS_SOURCE_BATCH:-1}"
...
validate_pi_window_selectors() {
  ...
  [[ "$FINAL_VERIFY_SECONDS" =~ ^[1-9][0-9]*$ ]] \
    || fail 'LIVE_FINAL_VERIFY_SECONDS must be a positive integer'
  [[ "$MAVROS_SOURCE_BATCH" =~ ^[01]$ ]] \
    || fail 'LIVE_MAVROS_SOURCE_BATCH must be 0 or 1'
  PI_HOLD_AFTER_WINDOW=1
...
print_pi_command() {
  ...
  printf '  exec env WORKSTATION_IP=%q LIVE_SSID=%q LIVE_RUN_SECONDS=%q LIVE_FINAL_VERIFY_SECONDS=%q LIVE_MAVROS_SOURCE_BATCH=%q LIVE_HOLD_AFTER_WINDOW=%q LIVE_FCU_TO_VRX_FANOUT=%q LIVE_ARMED_OBSERVATION=%q LIVE_ARMED_OBSERVATION_MAX_SECONDS=%q LIVE_ARMED_OBSERVATION_FINAL_SECONDS=%q LIVE_ARMED_OBSERVATION_STALE_SECONDS=%q LIVE_ARMED_OBSERVATION_LEFT_CHANNEL=%q LIVE_ARMED_OBSERVATION_LEFT_TRIM=%q LIVE_ARMED_OBSERVATION_RIGHT_CHANNEL=%q LIVE_ARMED_OBSERVATION_RIGHT_TRIM=%q HAILO_DEMO_ROOT="$PI_RUNTIME_ROOT" HAILO_LOCAL_DISPLAY=1 HAILO_LOCAL_WINDOW_MODE=%q "$PI_HELPER"\n' \
    "$WORKSTATION_IP" "$EXPECTED_SSID" "$RUN_SECONDS" \
    "$FINAL_VERIFY_SECONDS" "$MAVROS_SOURCE_BATCH" "$PI_HOLD_AFTER_WINDOW" \
