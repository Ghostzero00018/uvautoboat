...
| MAVROS Route 1 install on Pi 5 | 🟡 22/05/2026 — Ubuntu Noble + ROS apt path green; `ros-jazzy-mavros` 2.14.0, `ros-jazzy-mavros-extras`, and `ros-jazzy-mavros-msgs` installed from `packages.ros.org` with the dry-run dependency chain matching the real install. GeographicLib default datasets installed (`egm96-5`, `egm96`, `emm2015`) via corrected ROS 2 ament executable path `/opt/ros/jazzy/lib/mavros/install_geographiclib_datasets.sh`; `dialout` active after reboot. Late quick launch with `px4.launch fcu_url:=serial:///dev/ttyACM0:115200` exposed the expected `/mavros/*` plugin topic surface but `/mavros/state` stayed `connected: false` because no autopilot serial endpoint was visible (`/dev/ttyACM*`, `/dev/ttyUSB*`, `/dev/serial/by-id/*` absent; `lsusb` showed keyboard, mouse, RealSense only). Install / launch side green; physical MAVLink endpoint still missing. |
...
2. **Autopilot / boat physical connection to the Pi 5** — Pi install / MAVROS launch side is now green, but no CubePilot / Pixhawk / USB-UART endpoint is visible yet. This connection unblocks FCU connection-string selection, `/mavros/state connected: true`, and the first real Block C smoke-test. HDMI cabling does not count as a MAVLink telemetry path.
