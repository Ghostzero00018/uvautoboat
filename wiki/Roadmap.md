...
| 23/06/2026 | §3 refreshed and `wiki/YOLO_Dataset_Plan.md` added after the workstation YOLO toolchain smoke test and dataset-planning pivot in `working_diary/2026-06-23`: professor was not onsite, so QGC/Herelink/MAVROS live-test questions remain unanswered. Workstation GPU training is now the selected custom-detector path; Pi 5 stays capture + deployment-validation only. The smoke test proved CUDA train -> NCNN export with `yolo26n.pt` outside the public repo. Custom RealSense data capture, labeling, real-data training, Pi NCNN validation, ROS integration, and dashboard integration remain future work. |
