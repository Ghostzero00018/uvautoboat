...
| Hold-button pointer capture confirmed on hardware | 🟢 04/09/2026 — revision `8fda5a2` served the repaired dashboard while the unchanged certified `a8bed50` Pi bundle drove the T3a stack. Auto Move at `0.17` throttle, `1` s straight and a `0.10` right turn for `3` s stayed `ACTIVE` while the pressed pointer was dragged outside for at least two seconds; the operator observed both the fitted propellers and VRX continue until mouse-up. Mouse-up immediately restored `ARMED_NEUTRAL` and request zero; measured FCU output then returned to `800/800` us and VRX thrust to `0/0` N. Mouse and physical-propeller continuity are operator-observed; capture and W2 machine-correlate `10` active bursts and `587` enabled command frames with FCU output and VRX response. W2, W1, Pi and the wrapper closed cleanly, and the Pi archive is retained under `~/Desktop/test_logs_folder/pi_run_evidence/real_fcu_t3a_mouse_path_20260904_181410`. The capture verdict is red only because this focused run deliberately omitted both ESC-calibration annotations; it is not an ESC-calibration acceptance. |
| End-of-day parameter carry and power-off | 🟢 04/09/2026 — the operator confirmed `RC_OVERRIDE_TIME=0.5` and deliberately retained it through the weekend for Monday 07/09/2026's same-scope T3a video recording. This is not a fresh controller readback or a rollback. Verify `0.5` before reuse, then restore `3.0` with retained readback evidence after the video session completes or is abandoned. All hardware was operator-confirmed powered off on departure; no live approval carries forward. |
| Hailo kernel-update wording correction | 🟢 04/09/2026 — the earlier same-day row says the `1063` to `1064` update removed the driver. The `4.24.0` package and source remained installed; the new running kernel lacked matching headers and a built `hailo_pci` module. Installing the headers allowed DKMS to build the matching module. The six-minute recovery and run evidence are unchanged. |

### Blockers

**Current supersession - 04/09/2026:** the bridge-node decision, second ROS
consumer and dashboard-to-FCU command path in the older list below are no
longer blockers. The real-boat bench mode is selected by the printed bench URL
and documented in the runbook, while the live view remains labelled temporary
in the dashboard source, so the durable-promotion decision stays open. The
current near-term items are the planned Monday video session behind a fresh
declaration and approval, the
post-session-or-abandonment `RC_OVERRIDE_TIME=3.0` rollback evidence,
decoupling optional ESC annotations from the general T3a verdict, the Pi
kernel-headers meta-package, and full routine deployment acceptance. The
historical blocker trail remains below.
