...
| Install Mission Planner + QGroundControl on Linux workstation (prof-requested toolchain) | ✅ 24/04/2026 — MP 1.3.9384.38258 + QGC stable-daily 09/10/2025; MP-under-Mono GDAL / OGR / OSR degraded (Windows `.msi` fallback held for GIS demos). **11/05/2026 update**: MP-Linux video panel + arm/disarm path unblocked via host-local SkiaSharp 2.88.8 + `libdl.so` symlink fix (see `wiki/Common_Issues.md` MP-Linux entry); GDAL/OGR/OSR still degraded |
