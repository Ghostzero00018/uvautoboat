...
| MAVROS Route 1 install on Pi 5 | 🟡 22/05/2026 + 27/05/2026 + 28/05/2026 — Ubuntu Noble + ROS apt path green; `ros-jazzy-mavros` 2.14.0, `ros-jazzy-mavros-extras`, and `ros-jazzy-mavros-msgs` installed from `packages.ros.org` with the dry-run dependency chain matching the real install. GeographicLib default datasets installed (`egm96-5`, `egm96`, `emm2015`) via corrected ROS 2 ament executable path `/opt/ros/jazzy/lib/mavros/install_geographiclib_datasets.sh`; `dialout` active after reboot. Late 22/05 quick launch with `px4.launch fcu_url:=serial:///dev/ttyACM0:115200` exposed the expected `/mavros/*` plugin topic surface but `/mavros/state` stayed `connected: false`. 27/05 and 28/05 expanded Remmina-side audits found no physical MAVLink endpoint: no `/dev/serial/by-id/*`, `/dev/serial/by-path/*`, `/dev/ttyACM*`, `/dev/ttyUSB*`, or common UDP listener (`14550`, `14551`, `14540`, `5760`); only `/dev/ttyAMA10` appeared, which is not sufficient without TELEM wiring confirmation. 28/05 Slab 3 paper draft recorded the future Pi-side MAVROS `systemd` autostart shape with `fcu_url` intentionally left as a placeholder until a real endpoint appears. Install side green; physical MAVLink endpoint still missing. |
...
2. **Autopilot / boat physical connection to the Pi 5** — Pi install / MAVROS launch side is now green, but no CubePilot / Pixhawk / USB-UART / UDP MAVLink endpoint is visible yet as of the expanded 28/05/2026 Remmina-side audit. This connection unblocks FCU connection-string selection, `/mavros/state connected: true`, and the first real Block C smoke-test. HDMI cabling and a bare `/dev/ttyAMA10` node do not count as a MAVLink telemetry path without wiring confirmation.
