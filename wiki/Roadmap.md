...
| Hailo AI HAT+ 13 TOPS acceleration branch | 🟡 01/07/2026 + 02/07/2026 — Pi-side probe confirmed the mounted Hailo-8L board is PCIe-healthy (`1e60:2864`, gen-3 x1) on Ubuntu 24.04.4 / kernel `6.8.0-1057-raspi`, but no Hailo runtime was installed yet. The official pinned row is HailoRT / driver / pyHailoRT `4.24.0`, DFC `3.34.0`, and Model Zoo `2.19.0`. On the workstation, the official Docker suite loaded, reported HailoRT `4.24.0`, DFC `3.34.0`, and Model Zoo `2.19.0`, then compiled the custom `yolo26n` checkpoint to `yolo26n_route_a_six_heads.hef` for `HAILO8L` with six raw outputs and no embedded NMS. This closes the workstation compile gate only: Pi runtime install, DKMS/header proof, device execution, host-side decode, RealSense integration, and accuracy-grade calibration remain open. |
...
| 02/07/2026 | §3 gains the Hailo AI HAT+ 13 TOPS acceleration row after the official E2 artifact row and workstation compile gate closed. The Pi HAT is PCIe-healthy but still runtime-uninstalled; the workstation Docker suite produced `yolo26n_route_a_six_heads.hef` for `HAILO8L` with six raw outputs and no embedded NMS. Next gate is Pi runtime install plus device execution, not RealSense decode or dashboard integration. |
