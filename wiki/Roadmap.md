...
| VRX pose proof and current-source SITL reacceptance | 🟡 27/08/2026 — the exact W2 VRX launch produced eight transforms on `/wamv/pose`; `sydney_regatta -> wamv` was the sole world-parented transform and no child ended in `base_link`, directly proving the repaired selector. The full supervised SITL acceptance then passed on clean revision `147efe0` in `/home/ghostzero/Desktop/sitl_digital_twin_20260827_164623`. Independent adjudication checked ten hashes and passed the control cross-check, stop order, verdict, teardown, free ports, absent governed processes and final `SITL_ADJUDICATION=PASS`. This closes the current-source SITL evidence gate only. T0b remains open, its read-only snapshot pipeline is approved but **NOT RUN**, and Test B remains **NOT RUN**. |
