...
| MAVROS Route 1 install on Pi 5 | 🟡 22/05/2026 — Ubuntu Noble + ROS apt path green; `ros-jazzy-mavros` 2.14.0, `ros-jazzy-mavros-extras`, and `ros-jazzy-mavros-msgs` installed from `packages.ros.org` with the dry-run dependency chain matching the real install. GeographicLib default datasets installed (`egm96-5`, `egm96`, `emm2015`) via corrected ROS 2 ament executable path `/opt/ros/jazzy/lib/mavros/install_geographiclib_datasets.sh`; `dialout` active after reboot. Install side green; `/mavros/state` heartbeat remains deferred until autopilot / boat physical connection. |
...
2. **Autopilot / boat physical connection to the Pi 5** — Pi install side is now green; this connection unblocks FCU connection-string selection, `/mavros/state` heartbeat, and the first real Block C smoke-test.
