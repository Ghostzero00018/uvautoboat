...
2. **Real-topic integration from the now-proven MAVROS endpoint** — Pi install / MAVROS launch side is green, and the `/dev/ttyAMA0:57600` path is now proven through MAVProxy fanout into MAVROS with `/mavros/state connected: true` plus first ROS telemetry samples. The 10/06 QGC `.plan` import proves an offline dashboard-cache handoff only; it is not vehicle upload. Remaining blockers are GPS fix / EKF GPS configuration, targeted request/response timeout behaviour, command-path mapping, vehicle waypoint upload, and adapting the dashboard / simulation topic contract without regressing the default VRX stack. `/dev/ttyAMA10` remains a superseded silent-line result from before the reconfiguration.
