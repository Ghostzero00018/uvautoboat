...
2. **MAVROS pass on the now-evidenced endpoint** — Pi install / MAVROS launch side is green, and the professor photo sent on 03/06/2026, captured on 02/06 at 22:09, externally evidences a MAVProxy heartbeat on `/dev/ttyAMA0` at `57600`. The remaining blocker is proving the same path through MAVROS with `/mavros/state connected: true`, then capturing first ROS telemetry. `/dev/ttyAMA10` remains a superseded silent-line result from before the reconfiguration.
