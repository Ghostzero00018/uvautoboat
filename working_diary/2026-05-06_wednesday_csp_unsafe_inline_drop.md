...
20. `docs(diary): trim Thu Block B for D1+D2 hardware-only scope` — operator-facing scope clarification for tomorrow: Thu's Block B narrowed to B3 (deployment artifact bundle) only; B1 (sim sanity), B2 (sim-topic rosbag dry-run), and B4 (VRX HOLD re-eval) marked `> Skipped tomorrow` with reason inline (D1+D2 don't exercise the autonomy stack or VRX backend). Content kept in place for D3+ test windows; Block B time estimate dropped from ~30-60 min → ~10-15 min. Triggered by user clarification that the first wet test is purely hardware verification.
