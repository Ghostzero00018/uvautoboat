...
Interpretation at the 11:39 pass: MAVROS ROS-side low-level-controller link is now proven on 04/06/2026 because `/mavros/state connected: true` was captured and first ROS telemetry topics published. GPS fix / EKF GPS configuration, RC channel population, targeted request/response behaviour, command-path mapping, and dashboard / simulation integration remain open at this point.

**Live addendum, 04/06/2026 15:02-15:50:** Professor-requested combined-load check was attempted with the camera node, MAVProxy fanout, MAVROS, and ROS graph inventory in the same session. Treat this as a partial, power-limited run, not a clean combined pass.

Precheck: `imtaquadrone-desktop` was up for only 1 min at 15:02, consistent with the reported power-loss interruptions. RAM was healthy (15 GiB total, 1.2 GiB used, 13 GiB free, 14 GiB available, no swap used). The D435i remained on USB `5000M`; `/dev/ttyAMA0` existed as `root:dialout`; no stale MAVProxy / MAVROS / RealSense process was running. The precheck dmesg tail contained Wi-Fi-driver errors from `brcmf_proto_bcdc_*` / `brcmfmac` channel-setting attempts, not filtered RealSense failures.

Camera and MAVLink launch evidence: default RealSense launch opened depth `Z16 848x480x30` and color `RGB8 1280x720x30`, then reported `RealSense Node Is Up!`. MAVProxy on `--master=/dev/ttyAMA0 --baudrate 57600 --out=udpout:127.0.0.1:14550` detected vehicle `1:1`, `online system 1`, mode `MANUAL`, `fence present`, and repeated `AP: EKF3 waiting for GPS config data`; its `log_writer` thread later hit the same shutdown exception class as the earlier run. MAVROS `apm.launch fcu_url:=udp://127.0.0.1:14550@` detected remote address `1.1`, started UAS `MY ID 1.191, TARGET ID 1.1`, and logged `CON: Got HEARTBEAT, connected. FCU: ArduPilot` while the camera node was in the same run window.

Combined graph evidence: `ros2 node list` and `ros2 topic list -t` captured both `/camera/camera` and many `/mavros/*` entries. The topic list included RealSense color/depth topics (`/camera/camera/color/image_raw`, `/camera/camera/depth/image_rect_raw`, camera-info, metadata, and extrinsics), many MAVROS topics (`/mavros/imu/data`, `/mavros/global_position/raw/fix`, GPS status, geofence, rangefinder, and `/uas1/mavlink_source` / `/uas1/mavlink_sink`), plus unrelated TurtleBot/Create/Gazebo/OAK-D topics such as `/battery_state`, `/cmd_vel`, `/scan`, `/oakd/rgb/preview/image_raw`, and `/oakd/rgb/preview/depth`. Do not treat the full ROS graph as a clean AutoBoat-only inventory; Friday mapping must filter topics by source.

Camera rate during the combined attempt: `/camera/camera/color/image_raw` showed QoS `RELIABLE` / `TRANSIENT_LOCAL` with one publisher, then `timeout 20 ros2 topic hz` printed a transient "does not appear to be published yet" warning before reporting rates from `25.193` down to a final average of `19.245` Hz over a 138-message window. The command ended with `failed to shutdown: rcl_shutdown already called on the given context` after samples printed.

MAVROS echo result was not a clean pass in this combined attempt. The MAVROS launch log did contain `CON: Got HEARTBEAT, connected. FCU: ArduPilot`, but the plugin startup crawled: the endpoint opened around log stamp `1780580774`, while the heartbeat connection landed at `1780580966`, roughly 193 s later. The later `timeout 10 ros2 topic echo --once /mavros/state` returned `WARNING: topic [/mavros/state] does not appear to be published yet` and `Could not determine the type for the passed topic`. Subsequent echo attempts for `/mavros/imu/data`, `/mavros/global_position/raw/fix`, and `/mavros/rc/in` did not return samples in the pasted log; one `/mavros/rc/in` command was mistyped as `sensor_datata`, then the corrected command was interrupted. The MAVROS process had also been interrupted and escalated through SIGTERM to SIGKILL, so do not count this run as a fresh `/mavros/state` echo pass.

Power/stability result: the final filtered dmesg tail showed repeated `hwmon hwmon2: Undervoltage detected!` / `Voltage normalised` pairs from 15:46 through 15:49. This is the dominant result of the combined-load attempt: camera + MAVProxy + MAVROS can be launched together and appear together in the ROS graph, but the Pi 5 power rail is not stable enough to trust a full camera + MAVROS + topic-echo inventory run yet.

**Live addendum, 04/06/2026 MAVROS-only recovery test:** After the combined attempt, a MAVROS-only / no-camera follow-up recovered the ROS telemetry path. Two early `timeout 10 ros2 topic echo --once /mavros/global_position/raw/fix --qos-profile sensor_data` attempts returned no pasted sample, then direct `python3 stream_data.py` confirmed a MAVLink heartbeat and repeated raw IMU values; treat that script as direct MAVLink evidence, not a ROS telemetry pass. A later ROS echo on `/mavros/global_position/raw/fix` succeeded with `status: -1`, `service: 1`, `latitude: 0.0`, `longitude: 0.0`, `altitude: 17.163000000000004`, and covariance first entry `-1.0`, matching the no-fix GPS state.

The MAVROS-only `/mavros/state` echo then returned `connected: true`, `armed: false`, `guided: false`, `manual_input: true`, mode `MANUAL`, and `system_status: 4`, after several "A message was lost" notices. A mistyped `topic echo --once /mavros/imu/data` command failed because `topic` is not a command, but the corrected `ros2 topic echo --once /mavros/imu/data --qos-profile sensor_data` returned an IMU sample with frame `base_link` and linear acceleration `z: 9.77723005`. `/mavros/battery` returned `voltage: 15.368000984191895`, `percentage: 1.0`, and `present: true`. `/mavros/rc/in` returned `rssi: 255` plus 16 populated channels (`1515` on channels 1-5 and `875` on channels 6-16), also after "A message was lost" notices.

Interpretation of the MAVROS-only recovery: the `/dev/ttyAMA0:57600` + MAVProxy fanout + MAVROS path still works without camera load, and RC channel population is now observed in a no-camera run. This supports treating the professor-requested combined failure as a power/load stability problem, not as evidence that the MAVROS endpoint itself regressed. GPS still remains no-fix, and command/write-path validation remains open.

**Live addendum, 04/06/2026 clean MAVROS topology check:** A later process check showed the intended split topology with no direct Python serial consumer running: MAVProxy owned `/dev/ttyAMA0` at `57600` and fanned out to both `udpout:127.0.0.1:14550` and `udpout:127.0.0.1:14551`; MAVROS was launched with `fcu_url:=udp://127.0.0.1:14550@`; `mavros_node` was running under `/mavros`; no `stream_data.py` process was present. With that topology, `/mavros/state` returned `connected: true`, `armed: false`, `guided: false`, `manual_input: true`, mode `MANUAL`, and `system_status: 4`, after two "A message was lost" notices. This confirms the clean handoff rule for future tests: keep MAVProxy as the only serial owner, feed MAVROS from `14550`, and use `14551` only for optional direct-MAVLink inspection scripts.

**Live addendum, 04/06/2026 standalone direct-MAVLink script check:** `stream_data_enhanced.py` succeeded with the Python script as the sole serial consumer and no MAVProxy / MAVROS running. It received repeated HEARTBEAT messages (`MANUAL`, `armed: False`, `system_status: 4`), `RAW_IMU` / `SCALED_IMU2` / `SCALED_IMU3`, `GLOBAL_POSITION_INT`, `SYS_STATUS`, `BATTERY_STATUS` (about `15.13-15.19` V, `100%`), and populated `RC_CHANNELS` (`1515` on channels 1-5 and `875` on channels 6-16). GPS remained no-fix: `GPS_RAW_INT` reported `fix_type: 0`, `latitude: 0.0`, `longitude: 0.0`, altitude `0.0`, and `satellites visible: 0`. Treat this as direct-MAVLink evidence only, not a MAVROS / ROS telemetry pass.
...
**Outcome:** Wrap reopened for live Pi-side camera and MAVROS addendums after the original diary-only fallback. Durable runtime state did change today: MAVROS passed the ROS-side telemetry gate with `/mavros/state connected: true` and first ROS samples from IMU, raw GPS (no fix), battery, and RC topics. `Board.md` and `wiki/Roadmap.md` were updated for that state change. Later afternoon camera and combined-load addendums captured Herelink visual-only evidence, default RealSense color/depth rates, a partial camera + MAVROS graph inventory, and repeated under-voltage warnings; no numeric Herelink FPS / metadata was available, and the combined-load `/mavros/state` echo was not a fresh pass. A no-camera MAVROS-only recovery test then re-confirmed `/mavros/state connected: true`, IMU, battery, raw GPS (no fix), and populated `/mavros/rc/in` channels. A final topology check confirmed the clean split: MAVProxy owns serial, MAVROS consumes `14550`, optional scripts must use `14551`. A standalone `stream_data_enhanced.py` check also proved direct MAVLink telemetry from the controller when the script was the sole serial consumer, but this remains direct-MAVLink evidence only, not a ROS / MAVROS pass.

Final checks after the afternoon / combined-load / standalone direct-MAVLink addendums: `git status --short --branch` showed clean sync with one modified diary file; `git diff --check` passed; the changed-file visibility sweep over this diary returned no matches; the placeholder / conflict-marker scan matched only the check command embedded in this Block F checklist, not an unresolved outcome or conflict marker.

## Next steps

Friday 05/06/2026 startup: start from the proven MAVROS endpoint and the paper inventory above. First stabilize Pi power before relying on any combined camera + MAVROS run. If Pi 5 / Herelink access is available, capture numeric Herelink video stats only if the console exposes them; otherwise keep Herelink as visual-only evidence. For dashboard + full simulation stack integration inventory, filter the full ROS graph by source because the 04/06 combined snapshot also contained TurtleBot/Create/Gazebo/OAK-D topics. Re-capture `/mavros/state connected: true` and first MAVROS topic echoes while the camera node is still running before treating the combined path as validated. Do not edit Python / YAML / JavaScript until the user explicitly approves code/config changes.
