...
- **`0c7ae38` — Board.md milestone rows + dashboard CSP proposal section** (Fallback #3 research stage): two new 05/05 rows in `Board.md` (Block B sim verification + `--use-nvidia` 5-doc spread); `Last Updated` stamps bumped 04/05 → 05/05 at L11 + L373 (version 9.9 unchanged). New `### Proposed Content Security Policy (research, 05/05/2026)` section in `wiki/Dashboard_Security.md` under Recommended Mitigations: 7-resource external-load inventory + inline-content + eval scan (2 inline `<script>` blocks now at `index.html:11` + `:971` → `'unsafe-inline'` for `script-src`; 21 static `index.html` inline `style="…"` attrs plus additional runtime `app.js` style writes / generated style attrs → `'unsafe-inline'` for `style-src`; no `eval()` → no `'unsafe-eval'` needed) + proposed CSP header text + per-directive rationale table + 3 implementation paths (A wrapper / B reverse-proxy / C external server) + tightening path tied to Roadmap §1.3 vendoring + catches / doesn't-catch summary. L18 posture row gets a forward-pointer to the new section. +75 / -3.

- **`50ae2af` — CSP wrapper code, Option A end-to-end** (Fallback #3 implementation stage → done): new `web_dashboard/autoboat/serve_dashboard.py` (~50 LOC) subclassing `SimpleHTTPRequestHandler`, overrides `end_headers()` to inject `Content-Security-Policy` + `X-Content-Type-Options: nosniff` + `Referrer-Policy: strict-origin-when-cross-origin`. Launcher updated (L34 comment, L100 cleanup pkill pattern, L474 invocation); 5 doc spots swapped to the new canonical command (`README.md`, `USER_MANUAL.md`, `wiki/Quick_Start.md`, `web_dashboard/autoboat/README_autoboat_dashboard.md`, `wiki/Dashboard_Security.md`). Verified end-to-end post-launcher-restart on the live :8002: `curl -sI` shows all three security headers; dashboard loads + map tiles render + rosbridge connects (no new console regressions, only pre-existing Leaflet-library noise on `image-rendering` / `behavior` / `progid` parser warnings unrelated to CSP). +12 / -12 across 6 files + 1 new file.

**Mid-cycle review-pass fixes** caught + landed inside the same commits:

- **Item-4 + item-5 in Block C numbered list** contradicted the AM scope refinement (autonomy `/planning/emergency_stop` latching marked deferred in callout but item 5 still required dashboard-E-stop validation; full network-up check at item 4 same shape). Reworded item 5 → "Teleop stop / power cutoff test (release input → motor decel + physical kill path) — mandatory regardless of autonomy stack state"; reworded item 4 → "Network smoke check (only if dashboard / remote teleop is used today)". Full autonomy `/planning/emergency_stop` latching + full autonomy-stack network validation both deferred with D3-D5.
- **B3 deployment-bundle "Life preserver only if water entry allowed"** wrong framing for a lakeside test — rescue gear matters regardless of planned water entry. Reworded to "Life preserver / throw line per site rules".
- **`--use-nvidia` short callouts** in 3 user-facing docs over-generalized the campus-workstation-specific RTF / LiDAR figures as if universal. Trimmed to qualitative; detailed numbers stay attributed in `wiki/Common_Issues.md:600`.
- **CSP inline-content scan wording** grouped two distinct constraints (`<script>` blocks and `style="…"` attrs) under one bullet implying the same `'unsafe-inline'` directive gates both — split into two bullets for accuracy.
- **Board.md row B parenthetical** reduced the diary's nuanced item-4-and-5 narrowing to "autonomy E-stop + autonomy-stack network deferred". Tightened to "autonomy `/planning/emergency_stop` latching + full autonomy-stack network validation deferred — teleop stop/cutoff and a network smoke check stay mandatory".
- **`img-src` rationale row in CSP table** missing the field-deployment "append boat IP at `:8080`" note that `connect-src` had. Added parallel note.
- **"Option A first" sentence fragment** rephrased to a complete sentence ("Recommended: land Option A first ...").
- **`Referrer-Policy: no-referrer` over-aggression** — real regression caught browser-side: OSM volunteer tile servers gate on the `Referer` header per their usage policy, and `no-referrer` strips it entirely → "Access Blocked" tiles. Reverted to `strict-origin-when-cross-origin` (modern browser default; sends only the origin on cross-origin requests, satisfies OSM, still strips path/query). Verified by re-curl + browser hard-refresh: tiles load again, no `Access Blocked` error.

**Three more commits landed post-12:30:**

- **`ba6b5a3` — `docs(diary): log 05/05 fallback queue progress`** (12:30): committed this section's initial writeup (opening through review-pass fixes) plus the deferred-list cleanup that removed the now-completed Fallback #2 + #3 entries. Pure docs.

- **`852bca4` — `docs(security): note CSP wrapper effect on findings #2 + #3`** (12:50): two small additions to `wiki/Dashboard_Security.md`. Finding #2 (XSS via `addLog()`, resolved 04/05/2026): added a "Defense-in-depth (05/05/2026)" bullet noting CSP as the safety net underneath the renderer fix — even if a future regression re-introduced an `innerHTML` interpolation, CSP blocks scripts from non-allowed origins and inline-`eval` is excluded entirely. Finding #3 (Unencrypted WebSocket, still open): added a "Residual after 05/05/2026 CSP wrapper" bullet noting CSP `connect-src` allowlist now closes the XSS-redirect-WS vector but the underlying transport encryption is unchanged — anyone on the LAN can still passively sniff or actively MITM the rosbridge connection. +2 / -0.
