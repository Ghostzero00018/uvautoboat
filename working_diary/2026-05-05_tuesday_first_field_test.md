...
- **`0c7ae38` — Board.md milestone rows + dashboard CSP proposal section** (Fallback #3 research stage): two new 05/05 rows in `Board.md` (Block B sim verification + `--use-nvidia` 5-doc spread); `Last Updated` stamps bumped 04/05 → 05/05 at L11 + L373 (version 9.9 unchanged). New `### Proposed Content Security Policy (research, 05/05/2026)` section in `wiki/Dashboard_Security.md` under Recommended Mitigations: 7-resource external-load inventory + inline-content + eval scan (2 inline `<script>` blocks at `index.html:11` + `:977` → `'unsafe-inline'` for `script-src`; 21 inline `style="…"` attrs → `'unsafe-inline'` for `style-src`; no `eval()` → no `'unsafe-eval'` needed) + proposed CSP header text + per-directive rationale table + 3 implementation paths (A wrapper / B reverse-proxy / C external server) + tightening path tied to Roadmap §1.3 vendoring + catches / doesn't-catch summary. L18 posture row gets a forward-pointer to the new section. +75 / -3.
