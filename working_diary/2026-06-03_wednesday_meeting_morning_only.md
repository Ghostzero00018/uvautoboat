...
- No live boat telemetry was achieved on 02/06/2026. `/mavros/state connected: true` remains the pass condition.
...
- [ ] Confirm the morning-only scope and no planned afternoon work.

**Outcome:** [To fill - repo state, meeting readiness, morning-only confirmation.]

## Block B - 10h-12h meeting notes

- [ ] Keep visible story short:
  1. Progress since 20/05.
  2. Current dependency: real boat-data link still not proven.
  3. Ready vs pending.
...
- [ ] Use speaker notes for exact endpoint evidence:
  ...
  - Pi 5 GPIO UART mapping / `uart0-pi5` and `/dev/ttyAMA0` need exact post-config verification.
- [ ] Capture decisions:
...
Morning startup: verify clean/synced repo state, read the Tuesday endpoint proof, attend the 10h-12h meeting, and capture decisions. Afternoon is intentionally off-work unless explicitly reopened.
