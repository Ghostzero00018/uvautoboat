# Thursday 06/08/2026 - ArduPilot SITL and command-ingress design

> **PRE-DIARY - NOT STARTED.** Prepared at EOD 05/08/2026. This file does not
> authorize work. Every block below needs explicit approval before it starts.
> The day is workstation-only. No Pi, no control box, no FCU write, no arming,
> no motor or thrust command to real hardware, no edit to
> `tools/pi_live_hailo_mavlink_dashboard.sh`.

## Starting state

- Expected clean `main` with `HEAD == main == origin/main` and divergence
  `0/0`. Certify before starting; if `HEAD` is later than the revision that
  closed 05/08/2026, inspect every intervening commit first.
- Production pins unchanged from 04/08/2026:
...
- ArduPilot SITL, `sim_vehicle.py` and workstation MAVProxy are **absent**.
  `~/ardupilot` does not exist.
- Workstation `/` is at `94%`, `13 GB` free.
- The workstation is on `IoT IMT Nord Europe`, which carries the Pi ROS/SSH
  link and is not the campus internet path.
...
`reject_unexpected_command_subscribers` (`:902`) and `check_command_sentinel` at
ten-plus call sites abort a live run when a command service, unexpected command
subscriber, or monitored command message appears. Weakening a proven safety
boundary to add a write path is not an acceptable route. The dashboard likewise
stays `LIVE_MAVLINK_VIEW_ONLY=true`.
...
1. **Disk.** `13 GB` free at `94%`. An ArduPilot clone with submodules plus SITL
   build artifacts is multi-GB. Measure the requirement before committing to it.
   Regenerable-data cleanup is a **separate user-run gate**, not part of this
   block, and historical run evidence is retained.
2. **Network.** The clone needs real internet. The workstation is on
   `IoT IMT Nord Europe`, which is the Pi link. Campus internet is
   `IMT Nord Europe 5G`. Decide and record which SSID the install runs on, and
   note that switching drops the Pi link for the duration - acceptable today
   because no Pi work is planned.
...
Any ROS-side work in this block must not use the Pi's live domain; see the
domain note in the project's recorded configuration before starting a node.
